...
struct DBConnection
{
    ...
    UserRet addUser(string email, string username, string password, string name = "", string desc = "")
    {
	    ...
	    auto pass = toPassword(password.dup);
        auto hashpass = makeHash(pass);
    ...
    UserRet authUser(string email, string password)
    {
	    ...
	    auto dbpass = res["password"].toString().strip("\""); // the hashed password from database
        auto userSalt = parseHash(dbpass).salt; // the salt of the hashed and salted password
        auto inputPassHash = makeHash(toPassword(password.dup), userSalt); // we hash the given password with the salt
